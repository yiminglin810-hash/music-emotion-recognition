"""
Script to train and save DEAM baseline models
"""
import pandas as pd
import numpy as np
...
print("=" * 70)
print("🎯 DEAM Baseline Models Training and Saving")
print("=" * 70)

# Set paths
BASE_DIR = Path(__file__).parent.parent
DATA_DIR = BASE_DIR / 'data' / 'DEAM' / 'processed'
MODEL_DIR = BASE_DIR / 'models' / 'deam_baseline'
...
print(f"✅ Loaded {len(df)} songs with {len(df.columns)-3} features")
print(f"   Columns: {list(df.columns[:5])}... (showing first 5)")

# Prepare features and labels
feature_cols = [col for col in df.columns if col not in ['song_id', 'valence', 'arousal']]
...
ridge_valence = Ridge(alpha=1.0, random_state=RANDOM_STATE)
ridge_valence.fit(X_train_scaled, y_val_train)
ridge_val_pred = ridge_valence.predict(X_val_scaled)
ridge_val_r2 = r2_score(y_val_val, ridge_val_pred)
ridge_val_rmse = np.sqrt(mean_squared_error(y_val_val, ridge_val_pred))

print(f"✅ Valence: R² = {ridge_val_r2:.4f}, RMSE = {ridge_val_rmse:.4f}")

ridge_arousal = Ridge(alpha=1.0, random_state=RANDOM_STATE)
ridge_arousal.fit(X_train_scaled, y_aro_train)
ridge_aro_pred = ridge_arousal.predict(X_val_scaled)
ridge_aro_r2 = r2_score(y_aro_val, ridge_aro_pred)
ridge_aro_rmse = np.sqrt(mean_squared_error(y_aro_val, ridge_aro_pred))

print(f"✅ Arousal: R² = {ridge_aro_r2:.4f}, RMSE = {ridge_aro_rmse:.4f}")
...
models_to_save = [
    ...
    (ridge_valence, 'ridge_valence.pkl'),
    (ridge_arousal, 'ridge_arousal.pkl'),
...
summary = pd.DataFrame({
    'Model': ['Linear Regression', 'Linear Regression', 'Ridge', 'Ridge', 'Random Forest', 'Random Forest'],
    'Target': ['Valence', 'Arousal', 'Valence', 'Arousal', 'Valence', 'Arousal'],
    'R²': [lr_val_r2, lr_aro_r2, ridge_val_r2, ridge_aro_r2, rf_val_r2, rf_aro_r2],
    'RMSE': [lr_val_rmse, lr_aro_rmse, ridge_val_rmse, ridge_aro_rmse, rf_val_rmse, rf_aro_rmse]
})
...
print("\n" + "=" * 70)
print("✅ All DEAM models saved successfully!")
print("=" * 70)
